...
import { MISSION_UI } from './mission-ui';
import { createPlan } from './services/plan';
import { submitMissionAnswer } from './services/mission';
...
export default function AppV012() {
  ...
  const toggleMission = (id: string) => {
    const missions = state.plan!.missions.map((m) => m.id === id
      ? { ...m, done: !m.done, completedAt: !m.done ? new Date().toISOString() : undefined }
      : m);
    updatePlan({ ...state.plan!, missions });
  };

  const answerMission = async (id: string, answer: string) => {
    const plan = state.plan!;
    const mission = plan.missions.find((item) => item.id === id);
    if (!mission) return;
    const feedback = await submitMissionAnswer(state.profile!, plan, mission, answer);
    const currentIndex = plan.missions.findIndex((item) => item.id === id);
    const missions = plan.missions.map((item, index) => {
      if (item.id === id) {
        return { ...item, userAnswer: answer, aiFeedback: feedback.feedback, refinedOutput: feedback.refinedOutput };
      }
      if (feedback.nextMission && index === currentIndex + 1 && !item.done) {
        return { ...item, ...feedback.nextMission };
      }
      return item;
    });
    updatePlan({ ...plan, missions });
  };
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        {tab === 'today' && <Today state={state} copy={copy} onToggle={toggleMission} onAnswer={answerMission} onIncome={(amount) => setState((old) => ({ ...old, earnedIncome: old.earnedIncome + amount }))}/>} 
...
function initialProfile(): OnboardingData {
  ...
  return {
    ...
    weeklyHours: 7, skills: [], interests: [], experience: 'beginner', locale, market: marketCode,
    currency: market.currency, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC',
  };
}
...
function Today({ state, copy, onToggle, onAnswer, onIncome }: { state: AppState; copy: Copy; onToggle: (id: string) => void; onAnswer: (id: string, answer: string) => Promise<void>; onIncome: (v: number) => void }) {
  const plan = state.plan!;
  ...
  return <div className="page">
    ...
    {next ? <MissionCard mission={next} copy={copy} locale={profile.locale} onToggle={onToggle} onAnswer={onAnswer}/> : <div className="all-done"><Trophy/><h3>{copy.allDoneTitle}</h3><p>{copy.allDoneDesc}</p></div>}
...
function MissionCard({ mission, copy, locale, onToggle, onAnswer }: { mission: Mission; copy: Copy; locale: Locale; onToggle: (id: string) => void; onAnswer: (id: string, answer: string) => Promise<void> }) {
  const ui = MISSION_UI[locale];
  const [answer, setAnswer] = useState(mission.userAnswer || '');
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => { setAnswer(mission.userAnswer || ''); setError(''); }, [mission.id, mission.userAnswer]);

  const submit = async () => {
    if (!answer.trim()) { setError(ui.answerRequired); return; }
    setSubmitting(true); setError('');
    try { await onAnswer(mission.id, answer.trim()); }
    catch (cause) { setError(cause instanceof Error ? cause.message : copy.planError); }
    finally { setSubmitting(false); }
  };

  const suggestions = mission.suggestions || [];
  const question = mission.question || mission.description;

  return <article className="mission-card interactive-mission">
    <div className="mission-meta"><span className={`category c-${mission.category}`}>{copy.categories[mission.category]}</span><span><Clock3 size={14}/>{copy.minute(mission.minutes)}</span></div>
    <h2>{mission.title}</h2>
    <p>{mission.description}</p>

    {mission.why && <div className="mission-block why-block"><b>{ui.why}</b><p>{mission.why}</p></div>}
    {mission.deliverable && <div className="mission-goal-row"><div><b>{ui.deliverable}</b><span>{mission.deliverable}</span></div>{mission.successSignal && <div><b>{ui.successSignal}</b><span>{mission.successSignal}</span></div>}</div>}

    {!mission.aiFeedback && <>
      <div className="mission-question"><b>{question}</b></div>
      {suggestions.length > 0 && <div className="mission-suggestions"><span>{ui.aiIdeas}</span>{suggestions.map((item) => <button key={item} className={answer === item ? 'selected' : ''} onClick={() => setAnswer(item)}>{item}</button>)}</div>}
      <label className="mission-answer"><span>{ui.yourAnswer}</span><textarea value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder={ui.answerPlaceholder} rows={4}/></label>
      {error && <p className="mission-error">{error}</p>}
      <button className="complete ask-ai" disabled={submitting} onClick={submit}><Sparkles size={18}/>{submitting ? ui.askingAi : ui.askAi}</button>
    </>}

    {mission.aiFeedback && <div className="mission-feedback">
      <div className="feedback-head"><Sparkles size={18}/><b>{ui.aiReply}</b></div>
      <p>{mission.aiFeedback}</p>
      {mission.refinedOutput && <div className="refined-output"><b>{ui.refined}</b><p>{mission.refinedOutput}</p></div>}
      <div className="saved-answer"><b>{ui.yourAnswer}</b><p>{mission.userAnswer}</p></div>
      <button className="complete" onClick={() => onToggle(mission.id)}><span><Check/></span> {ui.continueAction}</button>
    </div>}
  </article>;
}
...
function SettingsView({ state, copy, onReset }: { state: AppState; copy: Copy; onReset: () => void }) {
  ...
  return <div className="page"><div className="page-title"><span>SETTINGS</span><h2>{copy.settingsTitle}</h2></div>
    ...
    <p className="version">v0.1.2 interactive beta</p>
  </div>;
}
